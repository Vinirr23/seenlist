...
/**
 * TASK-028 ÔÇö ganhou username (@handle), banner, e os 3 contadores
 * (item 4 ÔÇö "coment├írios" fica em 0 fixo, como a pr├│pria tarefa
 * autoriza: "podem permanecer zerados por enquanto", j├í que n├úo
 * existe feature de coment├írio nenhuma ainda). Bot├úo "Compartilhar
 * perfil" reaproveitado do componente j├í usado no perfil p├║blico.
 *
 * Tradu├º├úo (4┬║ lote) ÔÇö inclui o formatador de data ("Membro desde"),
      ...
      {profile?.bannerUrl && (
        <div className="relative -mx-4 h-56 w-[calc(100%+2rem)] overflow-visible">
          <div className="relative h-full w-full overflow-hidden rounded-b-lg bg-surface shadow-lg shadow-black/30">
            {/* eslint-disable-next-line @next/next/no-img-element -- banner externo, sem dom├¡nio fixo pra configurar em next/image */}
          ...
          {/**
           * Achado real (bug corrigido) ÔÇö a primeira vers├úo tentava
           * manter avatar+nome na MESMA linha flex, puxando a linha
           * inteira pra cima com margem negativa. Como o bloco de
           * texto (3 linhas) ├® mais alto que o avatar, alinhar pela
           * base (`items-end`) empurrava o TEXTO pra dentro da ├írea
           * da capa tamb├®m, n├úo s├│ o avatar ÔÇö ficava tudo espremido.
           * Agora o avatar ├® posicionado sozinho, ancorado na borda
           * de baixo da capa (`absolute -bottom-8`), e o nome/usu├írio
           * vira um bloco totalmente separado, abaixo, sem dividir
           * altura com nada ÔÇö mesmo padr├úo cl├íssico de capa +avatar
          ...
          <div className="absolute -bottom-8 left-4 flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-surface ring-2 ring-primary/50 ring-offset-2 ring-offset-background">
            {user.avatarUrl ? (
              // eslint-disable-next-line @next/next/no-img-element -- avatar externo, sem dom├¡nio fixo pra configurar em next/image
