...
/**
 * TASK-065 — mesmo padrão de `avatar-upload.ts`/`banner-upload.ts`
 * (bucket dedicado no Storage, path com o id do usuário, URL pública
 * de volta). Bucket "comment-images" precisa existir e ser público
 * no painel do Supabase (Storage → New bucket) — não é criado por
 * migration, mesma situação de "avatars" e "banners" hoje.
 */
export function useCommentImageUpload() {
  ...
  async function upload(file: File): Promise<string | null> {
    ...
    try {
      ...
      console.error(
        '[comments] Falha ao enviar imagem do comentário — confira se o bucket "comment-images" existe no Supabase Storage.',
        uploadError
      );
