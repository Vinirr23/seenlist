 ...
 *
 * CORREÇÃO (2026-08-27 — bug real, quebrou o build de produção; este
 * arquivo chegou a ser apagado por engano numa primeira tentativa,
 * achando (sem confirmar de verdade) que era código morto — não era:
 * `git grep` confirmou `runImport.ts` como único uso real) —
 * `decideWatchingVsUpToDate` (airDateCategory.ts) foi reescrita numa
 * sessão anterior ("motor resistente", investigação do Bleach) pra
 * decidir por IDENTIDADE de episódio (`watchedEpisodeKeys:
 * Set<string>`, chave "temporada-episódio"), não mais por um total
 * agregado — um total nunca prova QUAIS episódios específicos foram
 * vistos (uma importação bagunçada pode inflar a contagem). Esta
 * função ainda passava adiante só um número (`mainEpisodesWatched`),
 * o que não compila mais contra a nova assinatura. Corrigido pra
 * também receber o conjunto de chaves (`watchedEpisodeKeys`) —
 * `runImport.ts` já o constrói a partir de `reconstruction.episodes`
 * (a lista real, já reconstruída, de quais episódios entram como
 * assistidos) — mesmo padrão de identidade que os outros 3 lugares
 * que chamam `decideWatchingVsUpToDate` já usam. A checagem de
 * "assistiu tudo" (`allEpisodesWatchedOverall`) passa a usar
 * `watchedEpisodeKeys.size` no lugar do número antigo — equivalente,
 * já que o tamanho do conjunto é a contagem de episódios únicos.
 */
export function correctStatusWithLiveTmdb(
  ...
  const allEpisodesWatchedOverall = totalKnown > 0 && watchedEpisodeKeys.size >= totalKnown;
  ...
  if (liveEpisodes && liveEpisodes.length > 0) {
    const decision = decideWatchingVsUpToDate(watchedEpisodeKeys, liveEpisodes);
