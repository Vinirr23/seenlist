...
/** Sub-página "ver mais" de "Séries favoritas" no perfil público (`/u/[username]/favorite-series`). */
export function PublicFavoriteSeriesPageView({ username }: { username: string }) {
  ...
  return (
    <div className="w-full px-4 pb-24 pt-4 md:mx-auto md:max-w-[430px]">
      <SectionPageHeader title={t("profile.section.favoriteSeries")} backHref={`/u/${username}`} />
      <PublicFavoritesLibraryView userId={profile.userId} mediaType="series" viewModeScope="public-favorite-series" />
    </div>
  );
}
